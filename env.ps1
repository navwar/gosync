...
if (-Not ($path_parts -contains "$(Split-Path -parent $PSCommandPath)\powershell")) {
    Write-Host "Prepending local powershell directory to PATH: $(Split-Path -parent $PSCommandPath)\powershell"
    $env:PATH = "{0};{1}" -f "$(Split-Path -parent $PSCommandPath)\powershell", $env:PATH
}
